...
import { GetHistoryRecord } from '../servicies/Psych';
            ...
            <main className="flex-1 p-6 md:p-10 lg:p-12 max-w-7xl mx-auto w-full overflow-y-auto">
                <header className="mb-8 md:mb-10">
                    ...
                    <p className={`text-sm md:text-base ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'
                        ...
                        Acompanhe todos os prontuários criados sobre seus pacientes
                    </p>
                </header>
                ...
                {!isLoading && !apiError && records.length === 0 && (
                    <div className={`text-center py-16 rounded-2xl border ${isDarkMode ? 'bg-[#131c2e] border-gray-800' : 'bg-white border-[#8C5C32]/15'
                        ...
                        <p className={`text-base font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-[#4A2E14]'
                            ...
                            Você ainda não possui nenhum prontuário registrado.
                        </p>
                        <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-[#8C5C32]/70'
                            }`}>
                            Seus prontuátios criado apareção aqui.
                        </p>
                    </div>
                )}
                                ...
                                >
                                    ...
                                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 relative z-10">
                                        <div className="flex-1 space-y-4">
                                            <div>
                                                <h3 className={`text-lg font-bold tracking-wide flex items-center gap-2 ${isDarkMode ? 'text-gray-100' : 'text-[#4A2E14]'}`}>
                                                    {record.client_name || "Paciente não identificado"}
                                                </h3>
                                                <p className={`text-xs mt-1 font-medium uppercase tracking-wider ${isDarkMode ? 'text-[#A60321]' : 'text-[#A60321]'}`}>
                                                    {record.service_name || "Serviço não especificado"}
                                                </p>
                                            </div>

                                            <div className={`pt-3 border-t border-dashed ${isDarkMode ? 'border-gray-700' : 'border-[#8C5C32]/20'}`}>
                                                <p className={`text-sm leading-relaxed whitespace-pre-line ${isDarkMode ? 'text-gray-300' : 'text-[#4A2E14]'}`}>
                                                    {record.description || "Nenhuma descrição informada neste prontuário."}
                                                </p>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-5 pt-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`}>
                                                        Criado em: {record.format_date_br || "--"}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Brain className="w-4 h-4 text-[#A60321]" />
                                                    <span className={`text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`}>
                                                        Psicólogo(a): {record.psych_fullname || "Não informado"}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-col items-start lg:items-end gap-3 shrink-0">
                                            <span className={`text-[11px] font-semibold px-3 py-1.5 rounded-full uppercase tracking-wider border ${isDarkMode ? 'bg-gray-800 text-gray-300 border-gray-700' : 'bg-gray-100 text-[#4A2E14] border-gray-200'
                                                }`}>
                                                ID: #{record.id}
                                            </span>
                                        </div>
