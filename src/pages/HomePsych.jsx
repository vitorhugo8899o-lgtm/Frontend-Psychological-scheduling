...
import { GetNextsPsychAppoiments } from '../servicies/Psych';
import { useNavigate, Link } from "react-router-dom";
import SidebarButton from '../componentes/SidebarButton';

import { useTheme } from '../context/ThemeContext';

export default function HomePsych() {
    ...
    useEffect(() => {
        ...
        async function fetchAppointments() {
            try {
                ...
                const response = await GetNextsPsychAppoiments();
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mb-10">
                    <Link to="/avaliabilites">
                        ...
                        >
                            ...
                            >
                                Visualize seus horários de trabalho e gerencie
                            </p>
                        </div>
                    </Link>
                    ...
                    <Link to='/medical-record'>
                        ...
                        >
                            ...
                            <h3 className={`text-base font-medium mb-1.5 ${isDarkMode ? 'text-gray-200' : 'text-[#4A2E14]'}`}>Prontuários</h3>
                            <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`}>Acesse Prontuários dos seus pacientes</p>
                        </div>
                    </Link>
                    ...
                    <Link to='/dashboard'>
                        ...
                        >
                            <ChartSpline className="w-7 h-7 text-[#A60321] mb-4" />
                            <h3 className={`text-base font-medium mb-1.5 ${isDarkMode ? 'text-gray-200' : 'text-[#4A2E14]'}`}>Dashboard sobre sua consultas</h3>
                            <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`}>Veja dados sobre seu rendimento de consultas realizadas</p>
                        </div>
                    </Link>
                </section>
                ...
                <section className={`p-6 rounded-2xl border transition-colors duration-300 ${isDarkMode ? 'bg-[#131c2e] border-gray-800' : 'bg-white border-[#8C5C32]/15'
                    ...
                    {isLoading && (
                        <div className="flex flex-col items-center justify-center py-8 gap-3">
                            ...
                            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`}>Carregando suas próximas sessões...</span>
                        </div>
                    )}
                    ...
                    {!isLoading && !apiError && confirmedAppointments.length === 0 && (
                        <div className="text-center py-10">
                            <Calendar className={`w-10 h-10 mx-auto mb-3 opacity-40 ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`} />
                            <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-[#4A2E14]'}`}>Você não possui consultas agendadas no momento.</p>
                            <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-500' : 'text-[#8C5C32]/70'}`}>Mantenha suas disponibilidades atualizadas.</p>
                        </div>
                    )}
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                                            <div>
                                                <h4 className={`text-sm font-semibold tracking-wide ${isDarkMode ? 'text-gray-200' : 'text-[#4A2E14]'}`}>
                                                    Paciente: {appointment.client?.fullname || "Paciente não informado"}
                                                </h4>
                                                {appointment.client?.email && (
                                                    <p className={`text-xs font-medium mt-0.5 ${isDarkMode ? 'text-gray-400' : 'text-[#8C5C32]'}`}>
                                                        Contato: {appointment.client?.email}
                                                    </p>
                                                )}
                                            </div>
